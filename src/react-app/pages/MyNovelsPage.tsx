...
export default function NovelsPage() {
  ...
  const fetchNovels = async () => {
    try {
      ...
      console.error('获取感悟失败:', err);
      setError('获取感悟列表失败，请刷新重试');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        ...
        <p className="text-gray-600">加载感悟列表中...</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="border-l-4 border-purple-600 pl-4">
          <h1 className="text-3xl font-bold text-gray-900">科学学习圈</h1>
          <p className="text-gray-600 mt-2">精彩故事，科学人生，从这里开始</p>
        </div>
      ...
      {filteredNovels.length > 0 ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl border">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">暂无感悟</h3>
          <p className="text-gray-600 mb-6">还没有人创作感悟，成为第一个创作者</p>
          <Link
            to="/write/novel"
      ...
      {novels.length > 0 && (
        <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">感悟库统计</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-gray-900">{novels.length}</div>
              <div className="text-sm text-gray-600">感悟总数</div>
            </div>
            <div className="bg-white p-4 rounded-lg text-center">
