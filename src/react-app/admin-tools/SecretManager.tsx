...
const handleMouseUp = () => {
  ...
  try {
    const count = toolMode === 'custom' ? 1 : batchCount;
    const baseTitle = customTitle || "自动生成感悟";
                ...
                <div className="flex justify-between border-b pb-4">
                  <div>
                    ...
                    <div className="mt-2 flex items-center flex-wrap">
                      <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">💰 积分: {targetUser.coins}</span>
                ...
                {activeTab === 'novels' && (
                  <div className="pt-4">
                    {contentLoading ? (
                      ...
                      <div className="text-gray-500 text-center py-4">该用户未发布任何感悟</div>
                    ) : (
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-xs text-gray-500">
                          <tr>
  ...
  <div className="bg-white rounded-2xl shadow-sm border p-6">
    <div className="flex justify-between items-center mb-6">
      ...
      <div className="flex gap-2">
        ...
        >
          <BookOpen size={14} className="inline mr-1" /> 感悟
        </button>
      </div>
    </div>
          ...
          <tbody>
            {allContentData.map((item, index) => (
              ...
              >
                ...
                {contentTab === 'answers' && (
                  <>
                    <td className="p-2 truncate max-w-xs text-blue-600">{item.question?.title || '已发布评论'}</td>
